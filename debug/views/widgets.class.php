...
class twidget extends tevents {
  ...
  public function addtositebar($sitebar) {
    ...
    $id = $widgets->add($this);
    $view = tview::instance();
    $view->insert($id, false, $sitebar, -1);
...
$sitebars =tview::getsitebars($context);
...
$view = tview::instance($name);
$result->items = &$views->sitebars;
  }
...
$this->views->save();
  ...
  public function insert($id, $ajax, $index, $order) {
    if (!isset($this->view->sitebars[$index])) return $this->view->error("Unknown sitebar $index");
    $item = array('id' => $id, 'ajax' => $ajax);
    if (($order < 0) || ($order > count($this->view->sitebars[$index]))) {
      $this->view->sitebars[$index][] = $item;
    } else {
      array_insert($this->view->sitebars[$index], $item, $order);
    }
    $this->save();
  }
  ...
  public function delete($id, $index) {
    if ($i = $this->indexof($id, $index)) {
      array_delete($this->view->sitebars[$index], $i);
      $this->save();
      return $i;
    }
    return false;
  ...
  public function indexof($id, $index) {
    foreach ($this->view->sitebars[$index] as $i => $item) {
      if ($id == $item['id']) return $i;
    }
    return false;
  }
  ...
  public function move($id, $index, $neworder) {
    if ($old = $this->indexof($id, $index)) {
      if ($old != $newindex) {
        array_move($this->view->sitebars[$index], $old, $newindex);
        $this->save();
      }
    }
  }
