...
function uploadProgress(a,b,d){set_progress(Math.ceil(100*(b/d)),this.customSettings.progress)}function uploadError(){set_progress(-1,this.customSettings.progress);alert("uploadError")}function uploadComplete(){set_progress(-1,this.customSettings.progress)}function uploadSuccess(a,b){if("logourl"==this.customSettings.name){var d=$.parseJSON(b);$("#logo").css("padding-left",d.width);$("#text-color-logowidth").val(d.width);set_color("logourl",d.url)}else set_color(this.customSettings.name,b)}
