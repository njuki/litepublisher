...
typeof e)throw Error("The asynchronous onComplete handler callback function you provided is invalid; the value you provided ("+e.toString()+') is of type "'+typeof e+'".');}try{if(this.__isAsynchronous||this.__isCrossSite)rpc.pendingRequests[""+rpc.requestCount]={onSuccess:d,onException:c,onComplete:e};if(this.__isCrossSite){rpc.callbacks["r"+rpc.requestCount]=function(a,b){return function(c){c instanceof Object&&(c.result||c.error)?(c.id=b,a.__doCallback(c)):a.__doCallback({id:b,result:c})}}(this,
rpc.requestCount);var f=document.createElement("script");f.setAttribute("type","text/javascript");var g=this.__serviceURL+"/"+a+"?"+this.__callbackParamName+"=rpc.callbacks.r"+rpc.requestCount;b&&(g+="&"+rpc.toQueryString(b));f.setAttribute("src",g);f.setAttribute("id","rpc"+rpc.requestCount);var i=document.getElementsByTagName("head")[0];rpc.pendingRequests[rpc.requestCount].scriptElement=f;i.appendChild(f)}else{if(b&&(!(b instanceof Object)||b instanceof Date))throw Error("When making asynchronous calls, the parameters for the method must be passed as an array (or a hash); the value you supplied ("+
b+') is of type "'+typeof b+'".');var j,m;if("XML-RPC"==this.__protocol){if(!(b instanceof Array))throw Error("Unable to pass associative arrays to XML-RPC services.");d=['<?xml version="1.0"?><methodCall><methodName>'+a+"</methodName>"];if(b){d.push("<params>");for(f=0;f<b.length;f++)d.push("<param>"+this.__toXMLRPC(b[f])+"</param>");d.push("</params>")}d.push("</methodCall>");m=d.join("")}else j={version:"1.1",method:a,id:rpc.requestCount},b&&(j.params=b),m=this.__toJSON(j);var h;if(window.XMLHttpRequest)h=
new XMLHttpRequest;else if(window.ActiveXObject)try{h=new ActiveXObject("Msxml2.XMLHTTP")}catch(p){h=new ActiveXObject("Microsoft.XMLHTTP")}h.open("POST",this.__serviceURL,this.__isAsynchronous,this.__authUsername,this.__authPassword);"XML-RPC"==this.__protocol?(h.setRequestHeader("Content-Type","text/xml"),h.setRequestHeader("Accept","text/xml")):(h.setRequestHeader("Content-Type","application/json"),h.setRequestHeader("Accept","application/json"));if(this.__isAsynchronous){h.send(m);var l=this,
n=rpc.requestCount;h.onreadystatechange=function(){if(h.readyState==4){if(l.__protocol=="XML-RPC")try{var a=l.__getXMLRPCResponse(h,n)}catch(b){alert("XML-RPC Error! "+b.message)}else{a=l.__evalJSON(h.responseText,l.__isResponseSanitized);if(!a.id)a.id=n}l.__doCallback(a)}}}else{h.send(m);var k;k="XML-RPC"==this.__protocol?this.__getXMLRPCResponse(h,rpc.requestCount):this.__evalJSON(h.responseText,this.__isResponseSanitized);if(k.error)throw Error('Unable to call "'+a+'". Server responsed with error (code '+
k.error.code+"): "+k.error.message);this.__upgradeValuesFromJSON(k);return k.result}}}catch(o){if(a=!1,c&&(a=c(o)),e&&e(),!a)throw o;}};rpc.pendingRequests={};rpc.callbacks={};
...
b.push(g)}catch(i){b.push(g),b.push(i)}else b.push(g)}delete rpc.pendingRequests[a.id];if(b.length){for(var j,a="There "+(1==b.length?"was 1 uncaught exception":"were "+b.length+" uncaught exceptions")+": ",d=0;d<b.length;d++)d&&(a+="; "),a+=b[d].message,b[d].code&&(j=b[d].code);d=Error(a);d.code=j;throw d;}};
rpc.ServiceProxy.prototype.__toJSON=function(a){switch(typeof a){case "number":return isFinite(a)?a.toString():"null";case "boolean":return a.toString();case "string":var b={"\u0008":"\\b","\t":"\\t","\n":"\\n","\u000c":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\","/":"/"};return'"'+a.replace(/([\x00-\x1f\\"])/g,function(a,c){var d=b[c];if(d)return d;d=c.charCodeAt();return"\\u00"+rpc.zeroPad(d.toString(16))})+'"';case "object":if(null===a)return"null";if(a instanceof Array){for(var d=["["],c=0;c<a.length;c++)c&&
...
a.hasOwnProperty(e))1<d.length&&d.push(","),d.push(this.__toJSON(e)+":"+this.__toJSON(a[e]));d.push("}");return d.join("")}}throw new TypeError('Unable to convert the value of type "'+typeof a+'" to JSON.');};rpc.isJSON=function(a){return/^[,:{}\[\]0-9.\-+Eaeflnr-u \n\r\t]*$/.test(a.replace(/\\./g,"@").replace(/"[^"\\\n\r]*"/g,""))};
rpc.ServiceProxy.prototype.__evalJSON=function(a,b){var a=a.replace(/^\/\*-secure-([\s\S]*)\*\/\s*$/,"$1"),d;try{if(!b||rpc.isJSON(a))return eval("("+a+")")}catch(c){d=c}throw new SyntaxError("Badly formed JSON string: "+a+" ... "+(d?d.message:""));};
...
a[c][e].constructor);b+=encodeURIComponent(""+a[c][e])}else if(b&&(b+="&"),b+=encodeURIComponent(c)+"=",a[c]instanceof Date)b+=encodeURIComponent(rpc.dateToISO8601(a[c]));else{if(a[c]instanceof Object)throw Error("Unable to pass objects as parameters while in making a cross-site request. The object in question has this constructor: "+a[c].constructor);b+=encodeURIComponent(""+a[c])}return b};rpc.toArray=function(a){if(a instanceof Array)return a;for(var b=[],d=0;d<a.length;d++)b.push(a[d]);return b};
rpc.dateToISO8601=function(a){return a.getUTCFullYear()+"-"+rpc.zeroPad(a.getUTCMonth()+1)+"-"+rpc.zeroPad(a.getUTCDate())+"T"+rpc.zeroPad(a.getUTCHours())+":"+rpc.zeroPad(a.getUTCMinutes())+":"+rpc.zeroPad(a.getUTCSeconds())+"."+rpc.zeroPad(a.getUTCMilliseconds(),3)};rpc.zeroPad=function(a,b){b||(b=2);for(a=void 0==a?"":""+a;a.length<b;)a="0"+a;return a};
