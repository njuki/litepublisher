...
c.data("moder"));return!1},m=get_cookie("litepubl_user_id");a(c.buttons,b).each(function(){var b=a(this),h=b.data("idcomment");if(c.ismoder){if(a(d).appendTo(b).data("idcomment",h).data("moder","approved").click(g),a(e).appendTo(b).data("idcomment",h).data("moder","hold").click(g),a(f).appendTo(b).data("idcomment",h).data("moder","delete").click(g),a(l).appendTo(b).data("idcomment",h).data("moder","edit").click(g),b.is(":hidden"))a('<button type="button">E</button>').insertBefore(b).one("click",function(){a(this).next().show();
a(this).remove();return!1})}else if(b.data("idauthor")==m&&(c.canedit&&a(l).appendTo(b).data("idcomment",h).data("moder","edit").click(g),c.candelete&&a(f).appendTo(b).data("idcomment",h).data("moder","delete").click(g),(c.canedit||c.candelete)&&b.is(":hidden")))a('<button type="button">E</button>').insertBefore(b).one("click",function(){a(this).next().show();a(this).remove();return!1});k.onbuttons.fire(b)})}});a(m).ready(function(){get_cookie("litepubl_user_id")&&(litepubl.moderate=new litepubl.Moderate)})})(jQuery,
document,window);
