...
this.setenabled(!0);a(this.options.form).off("submit.moderate").on("submit.confirmcomment",function(){if("confirmcomment"in litepubl)return litepubl.confirmcomment.submit()})},loadhold:function(){var b=this,c=this.options;a(".loadhold").parent().remove();a.litejson({method:"comments_get_hold",idpost:ltoptions.idpost},function(d){try{if(c.ismoder){for(var e=a(c.comments),f=a(c.hold);e.next()[0]!=f[0];)e.next().remove();f.remove()}a(c.comments).after(d.items);b.create_buttons(c.hold)}catch(k){erralert(k)}}).fail(function(a,
c,f){b.error(lang.comments.errorrecieved)});return!1},create_buttons:function(b){var c=this.options,d=c.button.replace("%%title%%",lang.comments.approve),e=c.button.replace("%%title%%",lang.comments.hold),f=c.button.replace("%%title%%",lang.comments.del),k=c.button.replace("%%title%%",lang.comments.edit),l=this,g=function(){if(!l.enabled)return!1;var c=a(this);l.setstatus(c.data("idcomment"),c.data("moder"));return!1},m=get_cookie("litepubl_user_id");a(c.buttons,b).each(function(){var b=a(this),h=
b.data("idcomment");if(c.ismoder){if(a(d).appendTo(b).data("idcomment",h).data("moder","approved").click(g),a(e).appendTo(b).data("idcomment",h).data("moder","hold").click(g),a(f).appendTo(b).data("idcomment",h).data("moder","delete").click(g),a(k).appendTo(b).data("idcomment",h).data("moder","edit").click(g),b.is(":hidden"))a('<button type="button">E</button>').insertBefore(b).one("click",function(){a(this).next().show();a(this).remove();return!1})}else if(b.data("idauthor")==m&&(c.canedit&&a(k).appendTo(b).data("idcomment",
h).data("moder","edit").click(g),c.candelete&&a(f).appendTo(b).data("idcomment",h).data("moder","delete").click(g),(c.canedit||c.candelete)&&b.is(":hidden")))a('<button type="button">E</button>').insertBefore(b).one("click",function(){a(this).next().show();a(this).remove();return!1})})}});a(m).ready(function(){get_cookie("litepubl_user_id")&&(litepubl.moderate=new litepubl.Moderate)})})(jQuery,document,window);
