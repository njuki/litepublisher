...
{id:k,title:c.title,sizeclass:b,body:c.html,buttons:h}),e=this.dialog=a(k).appendTo("body"),b=0,d=c.buttons.length;b<d;b++)a("#"+g+b,e).data("index",b).on("click.dialog",c.buttons[b].click);e.on("shown.bs.modal",function(){a.isFunction(c.open)&&c.open(e);"tooltip"in a.fn&&a(".tooltip-toggle",e).tooltip({container:"body",placement:"auto top"})});e.modal();var l=this;e.on("hidden.bs.modal",function(){a.isFunction(c.close)&&c.close(e);l.dialog=!1;e.remove()});return e}});ready2(function(){"modal"in a.fn&&
(a.bootstrapDialog=new a.BootstrapDialog)})})(jQuery,window,document);
